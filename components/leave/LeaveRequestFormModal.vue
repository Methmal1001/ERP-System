        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg">
          <form @submit.prevent="handleSubmit">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div>
                <h2 class="text-lg font-bold text-gray-900">Request Leave</h2>
                <p class="text-sm text-gray-400 mt-0.5">Submit a new leave request</p>
              </div>
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <select v-model="form.leaveTypeId" required class="input bg-white">
                  <option value="" disabled>Select leave type</option>
                  <option v-for="type in activeLeaveTypes" :key="type.id" :value="type.id">
                    {{ type.name }} ({{ type.maxDaysPerYear }} days/yr, {{ type.isPaid ? 'Paid' : 'Unpaid' }})
                  </option>
                </select>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input v-model="form.endDate" type="date" required :min="form.startDate" class="input" />
                </div>
              </div>

              <p v-if="totalDays" class="text-xs text-gray-400">
                Duration: <span class="font-semibold text-gray-600">{{ totalDays }} day{{ totalDays === 1 ? '' : 's' }}</span>
              </p>
...
const activeLeaveTypes = computed(() => leaveStore.leaveTypes.filter((t) => t.isActive))
