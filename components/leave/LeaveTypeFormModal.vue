        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
          <form @submit.prevent="handleSubmit">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div>
                <h2 class="text-lg font-bold text-gray-900">New Leave Type</h2>
                <p class="text-sm text-gray-400 mt-0.5">Define a new category of leave</p>
              </div>
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input v-model="form.name" type="text" required class="input" placeholder="e.g. Annual Leave" />
              </div>

              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1.5">Max Days Per Year *</label>
                <input v-model.number="form.maxDaysPerYear" type="number" min="0" step="1" required class="input" placeholder="e.g. 14" />
              </div>

              <div class="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                <div>
                  <p class="text-sm font-medium text-gray-700">Paid Leave</p>
                  <p class="text-xs text-gray-400">Employees are compensated during this leave</p>
                </div>
                <button
                  type="button"
                  @click="form.isPaid = !form.isPaid"
                  :class="form.isPaid ? 'bg-blue-600' : 'bg-gray-300'"
                  class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors flex-shrink-0"
                >
                  <span :class="form.isPaid ? 'translate-x-6' : 'translate-x-1'" class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform"></span>
                </button>
              </div>
            </div>
...
const handleSubmit = async () => {
  ...
  const result = await leaveStore.createLeaveType({
    name: form.name.trim(),
    maxDaysPerYear: form.maxDaysPerYear ?? 0,
    isPaid: form.isPaid,
  })
