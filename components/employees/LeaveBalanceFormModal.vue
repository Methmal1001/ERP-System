        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
          <form @submit.prevent="handleSubmit">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div>
                <h2 class="text-lg font-bold text-gray-900">Set Leave Balance</h2>
                <p class="text-sm text-gray-400 mt-0.5">Assign or update an employee's leave allocation</p>
              </div>
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <select v-model="form.leaveTypeId" required class="input bg-white">
                  ...
                  <option v-for="type in leaveStore.leaveTypes" :key="type.id" :value="type.id">{{ type.name }}</option>
                </select>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block text-xs font-medium text-gray-600 mb-1.5">Total Days *</label>
                  <input v-model.number="form.totalDays" type="number" required min="0" step="0.5" class="input" />
                </div>
              </div>
            </div>
...
const handleSubmit = async () => {
  ...
  const result = await leaveStore.setLeaveBalance({
