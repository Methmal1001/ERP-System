<template>
  ...
  >
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
      ...
      <template v-if="visibleManagerItems.length">
        <NuxtLink
          v-for="item in visibleManagerItems"
          :key="item.to"
          :to="item.to"
          class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors group"
          :class="isActive(item.to)
...
const managerItems = [
  { to: '/attendance/approvals', label: 'Team Approvals', icon: icon('M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z') },
]
...
const visibleManagerItems = computed(() => (auth.isManager || auth.isCEO ? managerItems : []))
const visibleWorkforceItems = computed(() => (auth.isHrStaff ? workforceItems : []))
const visibleAdminItems = computed(() => (auth.isPrivileged ? adminItems : []))
