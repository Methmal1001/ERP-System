<template>
  <div class="p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
      <StatCard label="Employees" :value="format.number(employeesStore.totalCount)" hint="Total headcount" color="blue">
        <template #icon>
      ...
      <StatCard label="Departments" :value="departmentsStore.departments.length" :hint="`${departmentsStore.activeCount} active`" color="indigo">
        <template #icon>
      ...
      <StatCard label="Pending Leave" :value="format.number(leaveStore.pendingCount)" hint="Awaiting approval" color="red">
        <template #icon>
      ...
      <StatCard label="Present Today" :value="format.number(attendanceStore.presentCount)" :hint="todayLabelShort" color="emerald">
        <template #icon>
      ...
      <StatCard label="Payroll (Net)" :value="format.currency(payrollStore.totalNet)" :hint="currentMonthLabel" color="purple">
        <template #icon>
      ...
      <StatCard label="Avg. Rating" :value="performanceStore.averageRating ? `${performanceStore.averageRating} / 5` : '—'" hint="Performance reviews" color="amber">
        <template #icon>
...
onMounted(async () => {
  ...
  await Promise.all([
    employeesStore.fetchAll({ page: 1, pageSize: 1 }),
    departmentsStore.fetchAll(),
    leaveStore.fetchLeaveRequests(),
    attendanceStore.fetchAll({ from: todayIso, to: todayIso }),
    payrollStore.fetchAll({ month: today.getMonth() + 1, year: today.getFullYear() }),
