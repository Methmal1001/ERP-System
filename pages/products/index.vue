<template>
  <div class="min-h-screen bg-gray-50">
    <!-- Page Header -->
    <div class="bg-white border-b border-gray-100 px-4 sm:px-6 py-4 sticky top-0 z-10">
      <div class="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <div class="flex items-center gap-2.5">
            ...
            <div>
              <h1 class="text-xl font-bold text-gray-900 leading-tight">Products</h1>
              <p class="text-xs text-gray-400">
                {{ store.totalCount > 0 ? `${store.totalCount} products in inventory` : 'Inventory management' }}
              </p>
            </div>
          </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
          <div class="flex items-center justify-between mb-2">
            <p class="text-xs font-medium text-gray-400 uppercase tracking-wide">Page</p>
          ...
          <p class="text-2xl font-bold text-gray-900">{{ store.products.length }}</p>
          <p class="text-xs text-gray-400 mt-0.5">Showing now</p>
        </div>
        <!-- Active -->
        <div class="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
          <div class="flex items-center justify-between mb-2">
          ...
          <p class="text-2xl font-bold text-emerald-600">{{ store.activeCount }}</p>
          <p class="text-xs text-gray-400 mt-0.5">Products</p>
        </div>
        <!-- Inactive -->
        <div class="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
          <div class="flex items-center justify-between mb-2">
            <p class="text-xs font-medium text-gray-400 uppercase tracking-wide">Inactive</p>
          ...
          <p class="text-2xl font-bold text-red-500">{{ store.inactiveCount }}</p>
          <p class="text-xs text-gray-400 mt-0.5">Products</p>
        </div>
      </div>
      ...
      <Transition name="toast">
        <div
          ...
          class="flex items-center gap-3 px-4 py-3 border rounded-xl text-sm font-medium shadow-sm"
        >
          <svg v-if="toast.type === 'success'" class="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
            <polyline points="20 6 9 17 4 12" />
          </svg>
          <svg v-else class="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
...
const handleFormSuccess = () => {
  showToast(editingProduct.value ? 'Product updated successfully!' : 'Product added successfully!')
}
